...
(by default, it will be used to address the parts of the platform
which are being considered at any time in this document, unless specified
otherwise). 
...
the platform is that of business identity continuity; therefore, this
section also contains a set of design mock-ups that are to be kept
as reference during the development of the user interfaces.
...
form filling...). 
\item The customer-side web application (interface I1) must implement a
responsive design in order to fit most screen sizes. 
...
than 1 person (see requirement 4 in section \ref{sub:Functional-constraints})
\end{enumerate}
...
passenger added by a user (see requirement 1.(e) in section \ref{sub:Customer-side-application-specif})
\end{enumerate}
...
A more in-depth description of the use cases will be given in sections
...
the local queues and ride reservations scheduling in the platform.
It has access to all the software instances associated to \textit{ride,
taxi driver }and \textit{customer} objects and can modify these objects'
statuses in reaction to different events. 
...
\item If a taxi driver notifies a technical problem (function F7 of section
...
described in section \ref{sub:Actors-and-related}.
...
\textit{Queue manager }(entity E3, section \ref{sub:Entity-Behaviors})
...
The following platfrom metamodel was derived from the Alloy code in
...
authors, with a fair distribution of the mansions which cause each
